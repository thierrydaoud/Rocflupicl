...
module UnitTest_setup
    ...
    use ppiclf_data, only: x_per_min, x_per_max, y_per_min, y_per_max, z_per_min, z_per_max
    ...
    SUBROUTINE UT_setup
        ...
        ! this is awful, but im not ready to port all of the rocpicl user files yet.
        real*8 rpi,rmu,rkappa,rmass,vmag,rhof,dp,rep,rphip,rphif,asndf,rmachp,rhop,rhoMixt,reyL,rnu,fac,vx,vy,vz,rcp_part,rpr,phi, mp, re, rem
        common /RFLU_user/ rpi,rmu,rkappa,rmass,vmag,rhof,dp,rep,rphip, rphif,asndf,rmachp,rhop,rhoMixt,reyL,rnu,fac, vx,vy,vz, rcp_part,rpr, phi,mp,re, rem
        ! Setup Inputs
        !**********************************************************************
        ! DEFAULT VALUES
        ...
        IF(NumXCells*NumYCells*NumZCells .GT. PPICLF_LEE) THEN
            PRINT*, 'ERROR, PPICLF_LEE = ',PPICLF_LEE, 'Cells to test =', NumXCells*NumYCells*NumZCells
        ...
        IF(totalParticles .GT. PPICLF_LPART) THEN
            ...
            PRINT*, 'PPICLF_LPART =',PPICLF_LPART,'TotalParticles=', totalParticles
            CALL MPI_FINALIZE(ierr)
            STOP
        END IF
        ...
        DO j = 1,proc_ncells
            DO i = 4,6
                ...
                IF(p_grid(i,j) > filter(i-3)) filter(i-3) = p_grid(i,j)
        ...
        DO i = 1,3
            CALL MPI_Allreduce(filter(i),filter(i),1,MPI_DOUBLE, MPI_MAX,iComm,ierr)
            CALL MPI_Allreduce(dx_min(i),dx_min(i),1,MPI_DOUBLE, MPI_MIN,iComm,ierr)
        END DO
        ...
        IF(nid .EQ. rootProc) THEN 
            DO i = 1,totalParticles
                DO j = 1,3 ! loop through x, y, & z coordinates
                    ...
                    part_y(j,i) = gridDomain(1,j) + (gridDomain(2,j) - gridDomain(1,j))*randNum
                    ...
                    IF(i .LE. faceParticles) THEN
                        ! Choose face (k)
                        k = 3 !zface
                        IF(i .LT. INT(REAL(faceParticles*2.0/3.0)+0.49D0) + 1) k = 2 ! yface
                        IF(i .LT. INT(REAL(faceParticles*1.0/3.0)+0.49D0) + 1) k = 1 ! xface
                        ! only modify one coordinate to put on face 
                        IF(j .EQ. k) THEN
                            IF(i - (k-1)*INT(REAL(faceParticles/3.0)+0.49D0) .LT. INT(REAL(faceParticles/6.0)+1.49D0)) THEN
                    ...
                    IF(i .GT. faceParticles .AND. i .LE. INT(REAL(faceParticles)*(1.0D0+1.0D0/6.0D0))) part_y(j,i) = gridDomain(2,j) - pdia/2.0D0*randNum
                END DO ! j
            END DO ! i 
        END IF 
        ...
        CALL MPI_BCAST(part_y,totalParticles*SIZE(part_y,DIM=1), MPI_DOUBLE,rootProc,MPI_COMM_WORLD,ierr)
        ...
        DO i = 1,npart_local
            ...
            p_part_r(PPICLF_R_JVOLP,i) = (4.0D0/3.0D0)*PI*(0.5D0*pdia)**3 ! particle volume
            p_part_r(PPICLF_R_JSPL,i) = 1.0D0 ! Super Particle Loading 
        END DO

        nndistTemp  = 4.0D0*pdia
        CALL MPI_Allreduce(nndistTemp,nndist,1,MPI_DOUBLE, MPI_MAX,iComm,ierr)

        CALL MPI_BARRIER(icomm,ierr)

        RETURN
    END SUBROUTINE UT_setup

end module UnitTest_setup
!**********************************************************************
