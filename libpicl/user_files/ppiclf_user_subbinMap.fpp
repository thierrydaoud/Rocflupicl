...
module ppiclf_m_user_SubbinMap
    ...
    use ppiclf_m_user_ForceModels, only: ppiclf_user_BR_driver, ppiclf_user_Lift_driver
    ...
    SUBROUTINE ppiclf_user_subbinMap(i_Bin, n_SBin, tot_SBin,SBin_counter, SBin_map)
        ...
        INTEGER*4  SBin_map( 0 :                                                    &
            ((FLOOR((ppiclf_bins_dx(1)+2*ppiclf_nndist)/ppiclf_nndist) + 1) *       &
             (FLOOR((ppiclf_bins_dx(2)+2*ppiclf_nndist)/ppiclf_nndist) + 1) *       &
             (FLOOR((ppiclf_bins_dx(3)+2*ppiclf_nndist)/ppiclf_nndist) + 1) - 1),   &
            (ppiclf_npart+ppiclf_npart_gp))

        INTEGER*4  SBin_counter( 0 : (                                          &
            (FLOOR((ppiclf_bins_dx(1)+2*ppiclf_nndist)/ppiclf_nndist) + 1) *    &
            (FLOOR((ppiclf_bins_dx(2)+2*ppiclf_nndist)/ppiclf_nndist) + 1) *    &
            (FLOOR((ppiclf_bins_dx(3)+2*ppiclf_nndist)/ppiclf_nndist) + 1)      &
            - 1)                                                                &
            )
        ...
        DO l = 1,3
            i_Bin(l) = FLOOR((ppiclf_partpos(i)%pos(l) - ppiclf_binb(2*l-1)) /ppiclf_bins_dx(l))
        ...
        DO i = 1 , ppiclf_npart
            ...
            DO l = 1,3
                IF (l .LT. 3 .OR. ppiclf_ndim .GT. 2) THEN
                    i_SBin(l) = FLOOR((xp(l) - (bin_xMin(l) - ppiclf_nndist))/ppiclf_nndist) 
                ELSE
                    i_SBin(l) = 0
                END IF
            END DO
        ...
        RETURN
    END subroutine ppiclf_user_subbinMap
end module ppiclf_m_user_SubbinMap
