...
submodule (ppiclf_m_user_ForceModels) ppiclf_m_user_ForceModels_Fluctuations
    ...
    module procedure ppiclf_user_QS_fluct_Lattanzi
        ...
        Fs = 3.0*rpi*rmu*dp*(1.0+0.15*((rep*(1.0-rphip))**0.687))*(1.0-rphip)*vmag
        bq = min(sqrt(20.0*rmachp),1.0)*0.55*(rphip**0.7) *(1.0+tanh((rmachp-0.5)/0.2))
        sigF = (fq + bq)*Fs

        chi = (1.0+2.50*rphip+4.51*(rphip**2)+4.52*(rphip**3))/((1.0-(rphip/0.64)**3)**0.68)
        ...
        return
    end procedure ppiclf_user_QS_fluct_Lattanzi
    !
    !
    !-----------------------------------------------------------------------
    !
    ...
    module procedure ppiclf_user_QS_fluct_Osnes
        ...
        real*8 CD_average
        real*8 k_tilde, k_Mach, b_tilde, b_Mach, b_par, b_perp, k_Osnes, b_Osnes
        real*8 C1, C2, C3, C4, C5, D1, D2, D3, D4, D5, D6, D7, D8, E1, E2, E3, E4
        ...
        if(pseudoTurb_flag==1) then
            ...
            ! Mehrabadi's terms
            k_tilde = 2.0*phi + 2.5*phi*((1.0-phi)**3) * exp(-phi*(rem**0.5))                                 
            ...
            fit_func = -10.18530152*phi**3 + 10.94163073*phi**2 -7.07374862*phi +  0.38424203
            b_par = 0.523/(1.0+0.305*exp(-0.114*rem)) * exp(fit_func/(1.0+1.801*exp(-0.005*rem)))
            ...
            k_Mach = min(phi,0.3)*(-6.918*min(phi,0.3) + 2.238) * (tanh(C4/C5) + tanh((mp - C4)/C5))
            ...
            b_Mach = phi*(D9 + D10*phi + D11*phi**2) * (tanh(-D7/D8) - tanh((mp-D7)/D8))
            ...
            !--  Multiply by the mean relative flow kinetic energy to dimentionalize      
            ...
            A4 = H1 + H2*max(0.0d0, min(0.3d0, phi)) + H3*max(0.0d0, min(0.87d0, mp)) + H4*max(30.0d0, min(266.0d0, re))/300.0d0
  
            s_par = F8 + F9/(phi + F10) + F11 * mp
            !s_perp = G5/(phi + G6) + (G7*re)/(300.0*phi) + G8
            !---     We ditch Osnes's expression for s_perp and assume it as big as s_par
            s_perp = s_par
        
            ! 09/02/2025 - Cap according to Osnes model range
            s_T = H5 + H6*max(0.0d0, min(0.3d0, phi)) + H7*max(0.0d0, min(0.87d0, mp)) + H8*max(30.0d0, min(266.0d0, re))/300.0d0
            ...
            !--  Multiply Lagrangian Model by the Eulerian Mean Model
            R_par  = R_par  * Rmean_par 
            R_perp = R_perp * Rmean_perp

            !---  Q = [avec | bvec | cvec], 3x3 matrix
            !---  avec : unit vector in main direction
            !---  bvec, cvec: two orthogonal vectors to avec
            ...
            !---  R matrix only has diagonal components
            R(1,1) = R_par
            R(2,2) = R_perp
            R(3,3) = R_perp
  
            !--- Now Rotate the matrix, Rsg = Q . R . Q^T
  
            Rsg = matmul(Q, matmul(R,Qt))

            !--- Osnes Formulation for PTKE
        ...
        return
    end procedure ppiclf_user_QS_fluct_Osnes

end submodule ppiclf_m_user_ForceModels_Fluctuations
