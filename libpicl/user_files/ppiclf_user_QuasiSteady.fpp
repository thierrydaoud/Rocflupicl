...
submodule (ppiclf_m_user_ForceModels) ppiclf_m_user_ForceModels_QS
    ...
    contains
    module procedure ppiclf_user_QS_Parmar
        !
        ! Internal:
        !
        real*8 gamma
        real*8 rcd1,rmacr,rcd_mcr,rcd_std,rmach_rat,rcd_M1,rcd_M2,C1,C2,C3,f1M,f2M,f3M,lrep,factor,phi_corr
        ...
        if(re .lt. 1E-14) then
            ...
            rcd_mcr = (1.+0.15*re**(0.684)) + (re/24.0)*(0.513/(1.+483./re**(0.669)))
            if (mp .le. rmacr) then
                rcd_std = (1.+0.15*re**(0.687)) + (re/24.0)*(0.42/(1.+42500./re**(1.16)))
                rmach_rat = mp/rmacr
                rcd1 = rcd_std + (rcd_mcr - rcd_std)*rmach_rat
            else if (mp .le. 1.0) then
                rcd_M1 = (1.0+0.118*re**0.813) + (re/24.0)*0.69/(1.0+3550.0/re**.793)
                C1 =  6.48
                C2 =  9.28
                C3 = 12.21
                ...
                rcd_M1 = (1.0+0.118*re**0.813) + (re/24.0)*0.69/(1.0+3550.0/re**.793)
                rcd_M2 = (1.0+0.107*re**0.867) + (re/24.0)*0.646/(1.0+861.0/re**.634)
                C1 =  6.48
                C2 =  8.93
                C3 = 12.21
                f1M = -2.963 +4.392*mp -1.169*mp**2 -0.027*mp**3 -0.233*exp((1.0-mp)/0.011)
                f2M = -6.617 +12.11*mp -6.501*mp**2 +1.182*mp**3 -0.174*exp((1.0-mp)/0.010)
                f3M = -5.866 +11.57*mp -6.665*mp**2 +1.312*mp**3 -0.350*exp((1.0-mp)/0.012)
                lrep = log(re)
                factor = f1M*(lrep-C2)*(lrep-C3)/((C1-C2)*(C1-C3))  &
                        +f2M*(lrep-C1)*(lrep-C3)/((C2-C1)*(C2-C3))  &
                        +f3M*(lrep-C1)*(lrep-C2)/((C3-C1)*(C3-C2)) 
                ...
                rcd1 = (1.0+0.107*re**0.867) + (re/24.0)*0.646/(1.0+861.0/re**.634)
            end if ! mp
        endif    ! re
        ...
        return
    end procedure ppiclf_user_QS_Parmar
    !-----------------------------------------------------------------------
    ! Created Feb. 1, 2024
    ...
    !
    !-----------------------------------------------------------------------
    module procedure ppiclf_user_QS_ModifiedParmar
        !
        ! Internal:
        !
        real*8 gamma
        real*8 rcd1,rmacr,rcd_mcr,rcd_std,rmach_rat,rcd_M1,rcd_M2,C1,C2,C3,f1M,f2M,f3M,lrep,factor,phi_corr,b1,b2,b3
        ...
        b1 = 5.81*phi/((1.0-phi)**2) + 0.48*(phi**(1.d0/3.d0))/((1.0-phi)**3)

        b2 = ((1.0-phi)**2)*(phi**3)*re*(0.95+0.61*(phi**3)/((1.0-phi)*2))

        b3 = dmin1(sqrt(20.0d0*mp),1.0d0)*(5.65*phi-22.0*(phi**2)+23.4*(phi**3))*(1+tanh((mp-(0.65-0.24*phi))/0.35))
        ...
        return
    end procedure ppiclf_user_QS_ModifiedParmar
    !-----------------------------------------------------------------------
    !
    ! Created Feb. 1, 2024
    ...
    !-----------------------------------------------------------------------
    !
    module procedure ppiclf_user_QS_Osnes
        !
        ! Internal:
        !
        real*8 gamma,Knp,fKn,CD1,s,JM,CD2,cd_loth,CM,GM,HM,b1,b2,b3
        real*8 sgby2, JMt
        ...
        if (re .le. 45.0) then
            ...
            if (Knp > 0.01) then
                fKn = 1.0d0 / (1.0d0 + Knp*(2.514d0 + 0.8d0*exp(-0.55d0/Knp)))
            else
                fKn = 1.0d0 / (1.0d0 + Knp*(2.514d0 + 0.8d0*exp(-0.55d0/0.01)))
            ...
            if (mp <= 1) then
                ...
                JMt = 1.6d0*(mp**4) + 0.25d0*(mp**3) + 0.11d0*(mp**2) + 0.44d0*mp
            end if
            !
            ...
            cd_loth = CD1 / (1.0d0 + (mp**4)) +  CD2 / (1.0d0 + (mp**4))
        else
            ...
            if (mp < 1) then
                HM = 0.0239d0*(mp**3) + 0.212d0*(mp**2) - 0.074d0*mp + 1.d0
            else
                !HM =   0.93d0 + 1.0d0 / (3.5d0 + (mp**5))
                HM =   0.93967777777777772d0 + 1.0d0 / (3.5d0 + (mp**5))
            end if

            cd_loth = (24.0/re)*(1 + 0.15 * (re**(0.687)))*HM + 0.42*CM/(1+42500/re**(1.16*CM) + GM/sqrt(re))
        ...
        return
    end procedure ppiclf_user_QS_Osnes
    !-----------------------------------------------------------------------
    !
    ! Created Sep. 29, 2025
    !      
    ...
    !-----------------------------------------------------------------------
    !
    module procedure ppiclf_user_QS_Gidaspow
        !
        ! Internal:
        !
        real*8 phifRep, phif
        ...
        if(phif .lt. 0.8) then
            beta = 150.0*((phi**2)*rmu)/(phif * dp**2) + 1.75*(rhof*phi*vmag/dp)
        else 
        ...
        return
    end procedure ppiclf_user_QS_Gidaspow
    !-----------------------------------------------------------------------
end submodule ppiclf_m_user_ForceModels_QS