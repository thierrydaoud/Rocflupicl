...
submodule (ppiclf_m_user_ForceModels) ppiclf_m_user_ForceModels_BurnRate
    ...
    subroutine AL_CombModel(i,iStage,qq,mdot_me,mdot_ox)
        ...
        if (T_part .le. 933.0d0) then
            ...
            Cp_me = 28.08920d0 - 5.414849d0 * t + 8.560423d0 * t**2.0 + 3.427370d0 * t**3.0d0 - 0.277375d0 / t**2.0

            Cp_me = Cp_me / MW_me ! J/mol*K to J/kg*K
        else
            !liquid phase heat capacity (Shomate Equation)
            Cp_me = 31.75104d0 + 3.935826d-8 * t - 1.786515d-8 * t**2.0 + 2.694171d-9 * t**3.0d0 + 5.480037d-9 / t**2.0
        ...
        if (T_part .le. 2327.0d0) then
            !solid phase heat capacity (Shomate Equation)
            Cp_ox = 102.4290d0 + 38.7498d0 * t - 15.91090d0 * t**2.0 + 2.628181d0 * t**3.0d0 - 3.007551d0 / t**2.0

            Cp_ox = Cp_ox / MW_me2o3 ! J/mol*K to J/kg*K
        else
            !liquid phase heat capacity (Shomate Equation)
            Cp_ox = 192.4640d0 + 9.519856d-8 * t - 2.858928d-8 * t**2.0 + 2.929147d-9 * t**3.0d0 + 5.599405d-8 / t**2.0
...
end submodule ppiclf_m_user_ForceModels_BurnRate