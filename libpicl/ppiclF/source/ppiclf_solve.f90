...
module ppiclf_solve
    use mpi
    ! particle data
    use ppiclf_data, only: ppiclf_npart, ppiclf_y1
    use ppiclf_data, only: PPICLF_Y, PPICLF_YDOT, PPICLF_YDOTC, PPICLF_RPROP, PPICLF_RPROP2, PPICLF_RPROP3, PPICLF_RPROP4, PPICLF_RPROP5, PPICLF_FEEDBK, ppiclf_iprop
    ! grid data
    ...
    ! comm variables
    use ppiclf_data, only: ppiclf_comm, ppiclf_np, ppiclf_nid, ppiclf_cr_hndl
    ! binning variables
    use ppiclf_data, only: ppiclf_bins_dx, ppiclf_binb, ppiclf_bin_pos, ppiclf_previousbinb, ppiclf_n_bins
    ! ghost particle variables
    use ppiclf_data, only: ppiclf_npart_gp, ppiclf_particlemoved, ppiclf_cp_map, ppiclf_rprop_gp, ppiclf_iprop_gp
    ...
    use ppiclf_commu, only: ppiclf_comm_CreateBin, ppiclf_comm_FindParticle, ppiclf_comm_MoveParticle, ppiclf_comm_MapOverlapGrid, ppiclf_comm_MoveGhost, ppiclf_comm_CreateGhost
    ...
    public :: ppiclf_solve_Initialize
    public :: ppiclf_solve_InitParticle
    ...
    public :: ppiclf_solve_PeriodicParticleShift
    public :: ppiclf_solve_InvokeLinearPeriodic
    ...
    public :: ppiclf_solve_InterpTupleTransfer
    public :: ppiclf_solve_SBParticleToCellMap
    public :: ppiclf_solve_ParticleToCellMap
    public :: ppiclf_solve_Interpolate
    public :: ppiclf_solve_RemoveParticle
    public :: ppiclf_solve_ProjectParticleGrid
    ...
    SUBROUTINE ppiclf_solve_Initialize(xi1,xpmin,xpmax,yi1,ypmin,ypmax,zi1,zpmin,zpmax,ai1,apa,apxa,aprin,aprout)
        ...
        REAL*8    xpmin,xpmax,ypmin,ypmax,zpmin,zpmax,apa,apxa,aprin,aprout, pi, angled
    ...
    SUBROUTINE ppiclf_solve_InitParticle(imethod,ndim,iendian,npart,y,rprop,filt2,filt3)
        ...
        RETURN
    ENDSUBROUTINE ppiclf_solve_InitParticle
    ...
    SUBROUTINE ppiclf_solve_InitParam(imethod,ndim,iendian)
        ...
        IF(imethod .EQ. 0 .OR. imethod .GE. 3 .OR. imethod .LE. -2) CALL ppiclf_exittr('Invalid integration method$',0.0d0,imethod)
        IF(ndim .NE. 3) THEN
            PRINT*, 'ERROR: Rocpicl is written for 3D grids.'
            CALL ppiclf_exittr('Invalid problem dimension$',0.0d0,ndim)
        END IF
        IF(iendian .LT. 0 .OR. iendian .GT. 1) CALL ppiclf_exittr('Invalid Endian$',0.0d0,iendian)
    ...
    SUBROUTINE ppiclf_solve_AddParticles(npart,y,rprop)
        ...
        ! Internal:
        !
        INTEGER*4 ntotal,i,j
        !

        CALL ppiclf_prints('   *Begin AddParticles$')

        IF(ppiclf_npart+npart .gt. PPICLF_LPART .or. npart .lt. 0) CALL ppiclf_exittr('Invalid number of particles$',0.0D0,ppiclf_npart+npart)

        CALL ppiclf_printsi('      -Begin copy particles$',npart)

        CALL ppiclf_copy(ppiclf_y(1,ppiclf_npart+1), y, npart*PPICLF_LRS)
        CALL ppiclf_copy(ppiclf_rprop(1,ppiclf_npart+1), rprop, npart*PPICLF_LRP)
    ...
    END SUBROUTINE ppiclf_solve_AddParticles
    ...
    SUBROUTINE ppiclf_solve_NearestNeighborSB(i,SBt,SBc,SBm,SBn,iB)
        ...
        INTEGER*4 istride, k, kmax, kp, kkp, kk, j, jp, l, iSB, jSB, kSB, loopSB, tempSB, iSBin(3)
        ...
        DO iSB = 1,3     !to look at -1,current,+1 x-dir subbins
            DO jSB = 1,3   !to look at -1,current,+1 x-dir subbins
                DO kSB = 1,3 !to look at -1,current,+1 x-dir subbins
                    ...
                    loopSB = tempSB + (-2+iSB) + (-2+jSB)*SBn(1) + (-2+kSB)*SBn(1)*SBn(2)
        ...
        do j=1,ppiclf_nwall
            ...
            DO k=1,kmax 
                ...
                rdist = abs(rnx*ppiclf_cp_map(1,i)+rny*ppiclf_cp_map(2,i)+rnz*ppiclf_cp_map(3,i)+rd)
    ...
    SUBROUTINE ppiclf_solve_WriteVTU(time)
        ...
        RETURN 
    END SUBROUTINE ppiclf_solve_WriteVTU
    ...
    SUBROUTINE ppiclf_solve_IntegrateParticle(istep,iostep,dt,time)
        ...
        RETURN
    END SUBROUTINE ppiclf_solve_IntegrateParticle
    ...
    SUBROUTINE ppiclf_solve_IntegrateRK3s_Rocflu(iout)
        ...
        IF(ppiclf_linperiodic(1) .OR. ppiclf_linperiodic(2) .OR. ppiclf_linperiodic(3)) THEN
        ...
        RETURN
    END SUBROUTINE ppiclf_solve_IntegrateRK3s_Rocflu
    ...
    SUBROUTINE ppiclf_solve_PeriodicParticleShift
        ...
        RETURN
    END SUBROUTINE ppiclf_solve_PeriodicParticleShift
    ...
    SUBROUTINE ppiclf_solve_InvokeLinearPeriodic(i)
        ...
        IF(((x_per_flag.EQ.1).OR.(y_per_flag.EQ.1).OR.(z_per_flag.EQ.1)).AND.(ang_per_flag.EQ.0)) THEN

            DO j= 1,3
                ! particle leaving min. periodic face -> move it relative to 
                !                                         max periodic face
                IF(ppiclf_y(j,i) .LT. ppiclf_xdrange(1,j)) THEN
                    ppiclf_y(j,i) = ppiclf_xdrange(2,j) - ABS(ppiclf_xdrange(1,j) - ppiclf_y(j,i))
                    CYCLE
                END IF
                ...
                IF(ppiclf_y(j,i).GT.ppiclf_xdrange(2,j)) THEN
                    ppiclf_y(j,i) = ppiclf_xdrange(1,j) + ABS(ppiclf_y(j,i) - ppiclf_xdrange(2,j))
                END IF
            END DO ! j
        END IF 
 
        RETURN
    END SUBROUTINE ppiclf_solve_InvokeLinearPeriodic
    ...
    SUBROUTINE ppiclf_solve_InterpParticleGrid
        ...
        RETURN
    END SUBROUTINE ppiclf_solve_InterpParticleGrid
    ...
    SUBROUTINE ppiclf_solve_InterpTupleTransfer
        ...
        ! Internal: 
        !
        REAL*8 FLD(PPICLF_LEX,PPICLF_LEY,PPICLF_LEZ,PPICLF_LEE), Max_CellLen(3)
    ...
    SUBROUTINE ppiclf_solve_SBParticleToCellMap
        ...
        INTEGER*4 n_SBin(3), tot_SBin, i_SBin(3), iTemp_SBin(3), temp_SBin, iSB, jSB, kSB, loopSB, i_count, firstSB(3), lastSB(3)  
        ...
        DO l = 1,3
            ...
            n_SBin(l) = FLOOR((ppiclf_bins_dx(l) + 2*ppiclf_interp_dchk(l)) / ppiclf_interp_dchk(l)) + 1 
            dSQchk(l) = (ppiclf_interp_dchk(l))**2
            ! SB at bin min boundary
            firstSB(l) = FLOOR((ppiclf_bin_pos(1,l) - bin_Min(l)) / ppiclf_interp_dchk(l))
            ! SB at bin miax boundary
            lastSB(l)  =  FLOOR((ppiclf_bin_pos(2,l) - bin_Min(l)) / ppiclf_interp_dchk(l))
        END DO
        ...
        DO ie = 1,ppiclf_nCells_Interp  
            ...
            DO i = 0,1
                IF(i .EQ. 0) THEN
                    ...
                    IF(iTemp_SBin(1) .LT. 0) iTemp_SBin(1) = 0
                    IF(iTemp_SBin(1) .GT. n_SBin(1) - 1) iTemp_SBin(1) = n_SBin(1) - 1
                ELSE ! i .EQ. 1
                ...
                DO j = 0,1
                    IF(j .EQ. 0) THEN
                        ...
                        IF(iTemp_SBin(2) .LT. 0) iTemp_SBin(2) = 0
                        IF(iTemp_SBin(2) .GT. n_SBin(2) - 1) iTemp_SBin(2) = n_SBin(2) - 1
                    ELSE ! j .EQ. 1
                    ...
                    DO k = 0,1
                        IF(k .EQ. 0) THEN
                            ...
                            IF(iTemp_SBin(3) .LT. 0) iTemp_SBin(3) = 0
                            IF(iTemp_SBin(3) .GT. n_SBin(3) - 1) iTemp_SBin(3) = n_SBin(3) - 1
                        ELSE ! k .EQ. 1
                            ! This takes care of periodicity for single processor
                        ...
                        temp_SBin = iTemp_SBin(1) + iTemp_SBin(2)*n_SBin(1) + iTemp_SBin(3)*n_SBin(1)*n_SBin(2)
                        SBin_Counter(temp_SBin) = SBin_Counter(temp_SBin) + 1
                        IF(SBin_Counter(temp_SBin) .GT. ppiclf_nCells_Interp) PRINT*, 'counter more than interp cells. SB:', temp_SBin, SBin_Counter(temp_SBin), ppiclf_nCells_Interp
        ...
        DO ip=1,ppiclf_npart !Loop all particles in this bin
            ...
            END DO
            temp_SBin = i_SBin(1) + i_SBin(2)*n_SBin(1) + i_SBin(3)*n_SBin(1)*n_SBin(2)
            DO iSB = 1,3      ! -1,+0,+1 subbin in x-dir
                DO jSB = 1,3    ! -1,+0,+1 subbin in y-dir
                    DO kSB = 1,3  ! -1,+0,+1 subbin in z-dir
                        loopSB = temp_SBin + (-2+iSB) + (-2+jSB)*n_SBin(1) + (-2 + kSB)*n_SBin(1)*n_SBin(2)  
            ...
            IF (remove) THEN
                ...
                PRINT*, 'part # on proc # removed.', ppiclf_iprop(1,ip),ppiclf_nid
                remove = .FALSE.
            ELSE
    ...
    SUBROUTINE ppiclf_solve_ParticleToCellMap
        ! Local Variables
        INTEGER*4 i, j, k, l, ix, iy, iz, ip, ie, iee, nxyz, nnearest, CellID_nearest(28), partCount
        REAL*8    dSQl, dSQi, dSQ(28), xp(3), dSQchk(3), CellCenter(3,28), w(27),binblength(3), Max_CellLen(3),Max_CellLenSQ(3)
        LOGICAL   added, farAway

        !***************************************************************

        IF(ppiclf_nCells_Interp .EQ. 0 .AND. ppiclf_npart .GT. 0) THEN
            PRINT*,'No cells mapped to ppiclf bin. Num Particles/Proc ID:',ppiclf_npart, ppiclf_nid
            CALL ppiclf_exittr('Failure in particle to cell mapping',0.D0,0)
        END IF
        ...
        DO ip=1,ppiclf_npart !Loop all particles in this bin
            ...
            IF (nnearest .LT. 1) THEN
                ...
                ppiclf_iprop(9,ip) = -1
                ppiclf_remove_particle = .TRUE.
                PRINT*, 'part # on proc # removed with postion of:',ppiclf_iprop(1,ip),ppiclf_nid,xp(1),xp(2),xp(3) 
            ELSE
                partCount = partCount + 1
                ! use partCount since ip includes possible removed particles
                ppiclf_nPart2Cell(partCount) = nnearest
        ...
        RETURN
    END SUBROUTINE ppiclf_solve_ParticleToCellMap
    ...
    SUBROUTINE ppiclf_solve_Interpolate
        ...
        RETURN
    END SUBROUTINE ppiclf_solve_Interpolate
    ...
    SUBROUTINE ppiclf_solve_RemoveParticle
        ...
        DO i=1,ppiclf_npart
            IF(ppiclf_iprop(9,i) .NE. -1) THEN
                ...
                icount = icount + 1 
                IF(i .NE. icount) THEN
                    CALL ppiclf_copy(ppiclf_y     (1,icount),     ppiclf_y(1,i), PPICLF_LRS)
                    CALL ppiclf_copy(ppiclf_y1    (1,icount),    ppiclf_y1(1,i), PPICLF_LRS)
                    CALL ppiclf_copy(ppiclf_ydot  (1,icount),  ppiclf_ydot(1,i), PPICLF_LRS)
                    CALL ppiclf_copy(ppiclf_ydotc (1,icount), ppiclf_ydotc(1,i), PPICLF_LRS)
                    CALL ppiclf_copy(ppiclf_rprop (1,icount), ppiclf_rprop(1,i), PPICLF_LRP)
                    IF(PPICLF_LRP2 .GT. 1) THEN
                        CALL ppiclf_copy(ppiclf_rprop2(1,icount),ppiclf_rprop2(1,i),PPICLF_LRP2)
                    END IF
                    IF(PPICLF_LRP3 .GT. 1) THEN
                        CALL ppiclf_copy(ppiclf_rprop3(1,icount),ppiclf_rprop3(1,i),PPICLF_LRP3)
                    END IF
                    IF(PPICLF_LRP4 .GT. 1) THEN
                        CALL ppiclf_copy(ppiclf_rprop4(1,icount),ppiclf_rprop4(1,i),PPICLF_LRP4)
                    END IF
                    IF(PPICLF_LRP5 .GT. 1) THEN
                        CALL ppiclf_copy(ppiclf_rprop5(1,icount),ppiclf_rprop5(1,i),PPICLF_LRP5)
                    END IF
                    CALL ppiclf_copy(ppiclf_feedbk(1,icount), ppiclf_feedbk(1,i), PPICLF_LRP_PRO)
                    CALL ppiclf_icopy(ppiclf_iprop(1,icount) , ppiclf_iprop(1,i), PPICLF_LIP)
                END IF
    ...
    SUBROUTINE ppiclf_solve_ProjectParticleGrid
        ...
        DO ip=1,ppiclf_npart
            ...
            ! step at time = 0.0
            ppiclf_feedbk(PPICLF_P_JPHIP,ip) = ppiclf_rprop(PPICLF_R_JVOLP,ip) * ppiclf_rprop(PPICLF_R_JSPL,ip)
            ...
            y_norm = (ppiclf_y(PPICLF_JY, ip) - ppiclf_binb(3)) / (ppiclf_binb(4) - ppiclf_binb(3))
            z_norm = (ppiclf_y(PPICLF_JZ, ip) - ppiclf_binb(5)) / (ppiclf_binb(6) - ppiclf_binb(5))
            ...
            DO j=1,PPICLF_LRP_PRO
                ...
                DO i = 1,nCellProj
                    CellID = ppiclf_Part2Cell_map(ip,i)
                    ppiclf_pro_fld_picl(j,CellID) = ppiclf_pro_fld_picl(j,CellID) + ppiclf_feedbk(j,ip)*w(i)/wsum
                END DO !i
            END DO !j
        END DO !ip
