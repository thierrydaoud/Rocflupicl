...
!TODO: This isn't clear, so im just making it work for now, but it needs to be fixed at some point
    ...
    SUBROUTINE ppiclf_io_WritePerformance
        ...
        CALL MPI_Gather(localTime, 15, MPI_DOUBLE_PRECISION, TimeArray, 15, MPI_DOUBLE_PRECISION, 0, ppiclf_comm, ierr)

        CALL MPI_Gather(localParticles, 2, MPI_INTEGER4, Particles,      2, MPI_INTEGER4, 0, ppiclf_comm, ierr)

        CALL MPI_Gather(localCells, 2, MPI_INTEGER4, Cells,      2, MPI_INTEGER4, 0, ppiclf_comm, ierr)
        IF(ppiclf_nid .EQ. 0) THEN
            ActiveCnt = 0
            DO i = 1,ppiclf_np
                ActiveProc(i) = .FALSE.
            ...
            DO i = 1,15
                ...
                TminActive(i) = 1.0D22
                DO j = 1,ppiclf_np
                    IF(ActiveProc(j)) TminActive(i) = MIN(TimeArray(i,j),TminActive(i))
            ...
            DO i = 1,2
                ...
                DO j = 1,ppiclf_np
                    IF(ActiveProc(j)) PminActive(i) = MIN(Particles(i,j),PminActive(i))
            ...
            DO i = 1,2
                ...
                DO j = 1,ppiclf_np
                    IF(ActiveProc(j)) CminActive(i) = MIN(Cells(i,j),CminActive(i))
